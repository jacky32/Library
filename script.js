...
const newBookModalClose = document.getElementsByClassName(
  ...
  for (let i = 0; i < myLibrary.length; i++) {
    ...
    if (book.read == true) {
      mainContent.insertAdjacentHTML(
        "afterbegin",
        `<div class='book read' id='${book.id}'><div class='book-name'>${book.name} </div><div class='book-author'>${book.author}</div><div class='book-pages'>${book.pages} pages</div><button class='book-button-read'>Read?</button><button class='book-button-delete'>X</button></></div>`
      );
    } else {
      mainContent.insertAdjacentHTML(
        "afterbegin",
        `<div class='book' id='${book.id}'><div class='book-name'>${book.name} </div><div class='book-author'>${book.author}</div><div class='book-pages'>${book.pages} pages</div><button class='book-button-read'>Read?</button><button class='book-button-delete'>X</button></></div>`
      );
    }
  }
  addDeleteButtons();
